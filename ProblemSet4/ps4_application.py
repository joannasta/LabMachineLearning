...
def assignment_4():
	...
	C = np.arange(0.1,2.1,0.1)#np.ones(Sigmas.shape[0]) * 1/X_train.shape[0]#np.logspace(-7, 0, num=8, base=10)
...
def assignment_5():
    ...
    C = np.ones(Sigmas.shape[0]) * (1/X.shape[0])#np.arange(0,100,1)# kleines C wählen 1/n
    #params = {'kernel': ['gaussian'], 'kernelparameter': Sigmas, 'regularization': C}
    print(C.shape)
